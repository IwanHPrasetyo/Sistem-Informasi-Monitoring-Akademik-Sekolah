...
<body id="mimin" class="dashboard">
      ...
      <div class="container-fluid mimin-wrapper">
          ...
          $nama_siswa = $_SESSION['nama_sekolah'];
          $sql_siswa  = mysqli_query($conn,"SELECT * from tb_siswa where nama_siswa='$nama_siswa'");
          $data_siswa = mysqli_fetch_array($sql_siswa);
          $jurusan    = $data_siswa['id_jurusan'];
          $kelas      = $data_siswa['id_kelas'];
					
					$sql = "SELECT * FROM tb_jadwal as j join tb_pelajaran as p on j.kode_mapel = p.kode_mapel where j.npsn = '$npsn' and j.id_jurusan='$jurusan' and j.id_kelas = '$kelas'";
					$data = $conn->query($sql);
					while($row = $data->fetch_array()){
          
            $id_kelas    = $row['id_kelas']; 
            $sql_kelas   = mysqli_query($conn,"SELECT * FROM tb_kelas where id_kelas='$id_kelas'");
            $nama_kelas  = mysqli_fetch_array($sql_kelas);

            $id_jurusan  = $row['id_jurusan'];
            $sql_jurusan = mysqli_query($conn,"SELECT * FROM tb_jurusan where id_jurusan='$id_jurusan'");
            $nama_jurusan= mysqli_fetch_array($sql_jurusan);

					 ?>
					 <tr>
					 	             <td><?php echo $row['kode_jadwal'];?>
                         </td>
                         <td><?php echo $row['nama_pelajaran'];?>
                         </td>
                         <td><?php echo $row['jam_ke'];?>
                         </td>
                         <td><?php echo $row['hari'];?>
                         </td>
                         <td><?php echo $row['jam_mulai'];?>
					 	             </td>
                         <td><?php echo $row['jam_akhir'];?>
					 	             </td>
					 	            <td><?php echo $nama_kelas['nama_kelas'];?>
                        </td>
                        <td><?php echo $nama_jurusan['nama_jurusan'];?>
                        </td>
					 </tr>
					 <?php
					}
